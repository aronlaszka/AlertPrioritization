...
class DDPG(object):
    ...
    def _build_a(self, s, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Actor', reuse=reuse, custom_getter=custom_getter):
            h1 = tf.layers.dense(s, 32, activation=tf.nn.tanh, name='l1', trainable=trainable)
            #h2 = tf.layers.dense(h1, 32, activation=tf.nn.relu, name='l2', trainable=trainable)
            a = tf.layers.dense(h1, self.a_dim, activation=tf.nn.softmax, name='a', trainable=trainable)
            return a
    ...
    def _build_c(self, s, a, reuse=None, custom_getter=None):
        ...
        with tf.variable_scope('Critic', reuse=reuse, custom_getter=custom_getter):
            ...
            net2 = tf.layers.dense(net, 32, activation=tf.nn.relu, trainable=trainable)
    ...
    def learn(self, initial_state, state_observe, state_update, rnd=Random(0)):
        ...
        for i in range(MAX_EPISODES):
            ...
            for j in range(MAX_EP_STEPS):
                ...
                # an interesting test, first comment "self.ddpg.learn()" then run the folling code
                ...
                if self.ddpg.pointer > MEMORY_CAPACITY: #and j%5==0:
                    self.ddpg.learn()
        ...
        plt.xlabel('Episode')
        plt.ylabel('Average reward of each step')
        plt.show()
